...
that `mcp-guard fuzz` actually detects path-traversal.
...
- `MCPG025` (path traversal succeeded) — from `mcp-guard fuzz`.
- `MCPG031` (secret-shaped content in tool output) — from `mcp-guard fuzz`
